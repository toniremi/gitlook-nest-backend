...
export class GithubController {
   ...
   * Access via: GET /github/oauth/callback
   */
  @Get('oauth/callback')
   ...
   * Access via: GET /github/success?token=...
   */
  @Get('success')
  oauthSuccess(@Query('token') token: string) {
    return `<h1>GitHub OAuth Successful!</h1><p>Your Access Token: <code>${token}</code></p>
            ...
            <p>Try: <a href="/github/user?access_token=${token}">Get Authenticated User</a> (if you have a corresponding endpoint)</p>`;
  }

  /**
   * Example endpoint to get the authenticated user's profile.
   * Access via: GET /github/user?access_token=...
   ...
   * Access via: GET /github/repos/:owner/:repo?access_token=... (optional)
   */
